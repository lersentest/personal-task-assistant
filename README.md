...
В веб-интерфейсе добавлен модуль «Новости»:

- ежедневная сводка: `/news`;
- управление источниками: `/news/sources`;
- backend API: `/api/news/*`;
- таблицы PostgreSQL для источников, найденных материалов, выпусков и запусков обновления.

Первый слой использует seed-источники из `prisma/seed-data/news_sources_seed.json` и RSS/WEB-обработку.
X и Instagram импортируются выключенными со статусом `REQUIRES_API` до подключения официальных API.
Полные тексты статей не сохраняются: только метаданные, краткое описание и ссылка на оригинал.
...
src/
  ...
  database/     # Prisma
  news/         # источники, RSS/WEB-сбор, выпуски новостей
  projects/     # проекты
